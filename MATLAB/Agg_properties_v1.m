...
% workdir = pwd;
% addpath(workdir);
% addpath(workdir, 'matlab_test_data');




MainPath = ['/rds/general/user/mmorten/projects/smlm_oligomers/ephemeral/Cameron/Output/AD']; % Select the folder for averaged images
...
for folderN = 1:length(FolderList)
    ...
    figuredir = [MainPath ' Results/Figures/' FolderList(folderN).name '/ROI/']; % Destination for the figure saving; don't have to change the magenta part anymore (MM 17/11/20)
    
    datadir = [MainPath ' Results Properties/Data/' FolderList(folderN).name '/']; % Destination for the data saving; don't have to change the magenta part anymore (MM 17/11/20)
    ...
    mag = 1;                                        % Magnification of images during calculation was 10
    
    pixel_size = 12;                            % pixel size for length calculation (nm)
    
    RB_size = 3;                                 % Rolling-ball size in pixels, was 5.
    
    lnoise = 0;                                        % was 1 Level of noise size in pixels; usually 3. May be set to 0 or false, in which case only the highpass "background subtraction" operation is performed.
    
    lobject = 500;                                     % Level of object size in pixels somewhat larger than a typical object; usually 30-50, Syn=50, PrP=30 750
    
    th = 0.00001;                                   % Threshold for b/w conversion; usually 0.005-0.015. Highly dependent to the original image intensity. Was=0.00001
    
    save_status = 1;                              % Set 1 for saving figures/data; 0 for not saving.
    
    gaus = 1;                              % Gaus filter setting, was 3 (2)
